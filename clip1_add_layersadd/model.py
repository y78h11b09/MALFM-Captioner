...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, drop_r = 0.1):
        super().__init__()
        ...
        self.drop_path = DropPath(drop_r)
    ...
    def forward(self, x: torch.Tensor):
        if x.dim() == 3:
            ...
            z = x[:, -1, :, :]
            z = z + self.drop_path(self.attention(self.ln_1(z)))
            z = z + self.drop_path(self.mlp(self.ln_2(z)))
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        ...
        self.width = width
        self.layers = layers
        dpr = [x.item() for x in torch.linspace(0, 0.1, layers)]
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.permute(2, 1, 0, 3)  # LND -> NLD

        # x_cls = self.ln_post(x[:, 0, :])
        x_cls = checkpoint.checkpoint(self.ln_post, x[:, -1, 0, :])
