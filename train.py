...
from tf_adpt_fusion import GPT2LMHeadModel
...
from clip1_add_layersadd import clip
from clip1_add_layersadd.clip import _transform
import math
...
from misc_fusion import generate2_adpt_if, evaluate_on_coco_caption
import torch.nn.functional as F

from models.transformer import MemoryAugmentedEncoder, ScaledDotProductAttentionMemory, ScaledDotProductAttention
# from data import ImageDetectionsField, TextField, RawField
# from data import COCO, DataLoader
import warnings
import shutil
import h5py
GPT2_LOCAL_PATH = "/home/amax/.cache/huggingface/transformers/models--gpt2/snapshots/6076b4b6f4c6b0b9b8c8a2c1d3e4f5a6b7c8d9e0f"
...
class ClipCocoDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        try:
            ...
            precomp_data = f['%d_features' % img_id][()]
            if self.sort_by_prob:
                precomp_data = precomp_data[np.argsort(np.max(f['%d_cls_prob' % img_id][()], -1))[::-1]]
        except KeyError:
            warnings.warn('Could not find detections for %d' % img_id)
        ...
        return tokens, mask, image, precomp_data, gt

    def __init__(self, data_root: str, data_path: str, detections_path: str, max_detections,
                 gpt2_type: str = "gpt2", len_seq=20, load_in_tmp=True, normalize_prefix=False, sort_by_prob=False):
        self.data_root = data_root

        self.tokenizer = GPT2Tokenizer.from_pretrained("/home/amax/.cache/huggingface/transformers/models--gpt2/snapshots/6076b4b6f4c6b0b9b8c8a2c1d3e4f5a6b7c8d9e0f")

        self.normalize_prefix = normalize_prefix
        self.preprocess = _transform(224)  # add turn over
...
class ClipCocoValDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        try:
            f = h5py.File(self.detections_path, 'r')
            precomp_data = torch.from_numpy(f['%d_features' % img_id][()])
            if self.sort_by_prob:

                idxs = torch.from_numpy(np.argsort(np.max(f['%d_cls_prob' % img_id][()], -1))[::-1])
        ...
        return image, _filename, tokens, gt, mask, precomp_data

    def __init__(self, data_root: str, detections_path: str, max_detections, load_in_tmp=True, sort_by_prob=False):
        self.data_root = data_root


        self.tokenizer = GPT2Tokenizer.from_pretrained(
            "/home/amax/.cache/huggingface/transformers/models--gpt2/snapshots/6076b4b6f4c6b0b9b8c8a2c1d3e4f5a6b7c8d9e0f")
        self.annotation = json.load(
            open("/media/amax/date1/song/coco2014/captions_train-val2014/annotations/captions_val2014.json", "r"))[
            "annotations"]
...
class ClipCaptionModel(nn.Module):
    ...
    def forward(self,tokens: torch.Tensor, mask_tokens: torch.Tensor, prefix: torch.Tensor,
                precomp_data: torch.Tensor, mask: Optional[torch.Tensor] = None, t=None, epoch=None,
        ...
        # with torch.no_grad():
        #     vit_feature, len_cls = self.image_encode(prefix)
        # mm_feature, mask_enc = self.encoder(precomp_data)
        # prefix_projections = self.clip_project(vit_feature)
        # prefix_projections_mm = self.clip_project_mm(mm_feature)
        # if self.training:
        #     empty_idx = int(self.if_drop_rate * batch_size)
        #     for i_image in range(empty_idx):
        #         prefix_projections[i_image, :, :] = self.pad_embedding
        #         prefix_projections_mm[i_image, :, :] = self.pad_embedding_mm
        if self.training:
            with torch.no_grad():
                vit_feature, len_cls = self.image_encode(prefix)
            mm_feature, mask_enc = self.encoder(precomp_data)
            # start_time = time.time()
            vit_feature_noise, mm_feature_noise = self.add_noise(vit_feature, mm_feature, epoch)
            # end_time = time.time()
            # print(f"add noise time: {(end_time - start_time) / 100}")
            prefix_projections = self.clip_project(vit_feature_noise)
            prefix_projections_mm = self.clip_project_mm(mm_feature_noise)
            empty_idx = int(self.if_drop_rate * batch_size)
            for i_image in range(empty_idx):
                prefix_projections[i_image, :, :, :] = self.pad_embedding
                prefix_projections_mm[i_image, :, :, :] = self.pad_embedding_mm
        ...
        out = self.gpt(t=t, inputs_embeds=embedding_text, labels=labels, attention_mask=mask,
                       encoder_hidden_states=prefix_projections, encoder_hidden_states_mm=prefix_projections_mm)
        return out, self.len_head(len_cls)

    def __init__(self, prefix_length: int, encoder, add_noise, clip_length: Optional[int] = None, prefix_size: int = 512,
                 num_layers: int = 8, mapping_type: MappingType = MappingType.MLP, Timestep: int = 20, if_drop_rate=0.02):
        super(ClipCaptionModel, self).__init__()
        self.prefix_length = prefix_length
        # self.gpt = GPT2LMHeadModel.from_pretrained('gpt2')
        gpt2_local_path = "/home/amax/.cache/huggingface/transformers/models--gpt2/snapshots/6076b4b6f4c6b0b9b8c8a2c1d3e4f5a6b7c8d9e0f"
        configuration = GPT2Config.from_pretrained(gpt2_local_path)
        ...
        self.pad_embedding = nn.Parameter(torch.randn(size=(12, 196, 768), requires_grad=True, dtype=torch.float64)) # image free vector
        self.pad_embedding_mm = nn.Parameter(torch.randn(size=(3, 50, 768), requires_grad=True, dtype=torch.float64)) # image free vector
        self.len_head = MLP((512, self.gpt_embedding_size // 2, 20))
        if mapping_type == MappingType.MLP:
            self.clip_project = MLP((768, self.gpt_embedding_size // 2,
                                     self.gpt_embedding_size))
            self.clip_project_mm = MLP((768, self.gpt_embedding_size // 2,
                                     self.gpt_embedding_size))
        else:
...
def val(model, epoch, val_dataloader, args):
    ...
    for idx, (image, image_path, tokens, gt, mask, precomp_data) in enumerate(val_dataloader):
        image = image.cuda(non_blocking=True)
        tokens = tokens.cuda(non_blocking=True)
        mask = mask.cuda(non_blocking=True)
        gt = gt.cuda(non_blocking=True)
        precomp_data = precomp_data.cuda(non_blocking=True)
        for i_t in range(0, model.module.time_step):
            ...
            outputs, len_out = model(tokens, mask_tokens, image, precomp_data, all_mask, t)
        ...
        vit_feature, len_cls = model.module.image_encode(image)
        mm_feature, _ = model.module.encoder(precomp_data)
        prefix_embed = model.module.clip_project(vit_feature)
        prefix_embed_mm = model.module.clip_project_mm(mm_feature)
        len_pre = model.module.len_head(len_cls)
        if args.use_beam_search:
            assert False, "Not check beam search for now"
            generated_text_prefix = generate_beam(model, tokenizer, embed=prefix_embed)[0]
        else:
            generated_text_prefix = generate2_adpt_if(model, tokenizer, embed=prefix_embed, embed_mm=prefix_embed_mm,
    ...
    if dist.get_rank() == 0:
        ...
        result = evaluate_on_coco_caption(result_all,
                                          ...
                                          os.path.join(args.data_root, 'captions_train-val2014/annotations/captions_val2014.json'))
    else:
        result = None
    torch.distributed.barrier()
    if dist.get_rank() == 0:
...
def train(model, epoch, train_dataloader, optimizer, lr_scheduler, scaler, args,
    ...
    for idx, (tokens, mask, prefix, precomp_data, gt) in enumerate(train_dataloader):
        ...
        with amp.autocast(enabled=args.enable_amp):
            outputs, len_out = model(tokens, mask_tokens, prefix, precomp_data, all_mask, t, epoch=epoch)
        logits = outputs.logits
        loss_len = nnf.cross_entropy(len_out, mask.sum(dim=-1).to(torch.long) - 1)
        loss = nnf.cross_entropy(logits.reshape(-1, logits.shape[-1]), gt.flatten(), ignore_index=-1)
        loss = loss + loss_len
...
def parse_args():
    ...
    parser.add_argument('--data_root', default='/media/amax/date1/song/coco2014/', help='raw coco training image path')
    ...
    parser.add_argument('--m', default=40)
    # parser.add_argument('--exp_name', type=str, default='m2_transformer')
    # parser.add_argument('--head', type=int, default=8)
    parser.add_argument('--resume_last', action='store_true')
    parser.add_argument('--resume_best', action='store_true')
    parser.add_argument('--features_path', type=str,
                        default='/home/amax/桌面/S2-Transformer-main/COCO2014_VinVL.hdf5')
    parser.add_argument('--annotation_folder', type=str,
                        default='/media/amax/date1/datasets/coco2014/meshed_memory/annotations')
...
def add_noise(feature, feature_mm, epoch, mean=0.0, std=0.1):
    bs = feature.size(0)
    if epoch < 10:
        # return feature, feature_mm
        prob = 0.4
    elif epoch < 20:
        prob = 0.2
    else:
        ...
        return feature, feature_mm
    # print(prob)
    noise = torch.randn_like(feature[0, 0, :, :]) * std + mean
    device = feature.device
    mask = torch.bernoulli(torch.full((196, 768), prob))
    mask = mask.to(device)
    mask_noise = torch.mul(noise, mask)
    mask_noise = mask_noise.unsqueeze(0).repeat(12, 1, 1)
    mask_noise = mask_noise.unsqueeze(0).repeat(bs, 1, 1, 1)
    mask_noise_mm = mask_noise[:, :3, :50, :]
    feature = feature + mask_noise
    feature_mm = feature_mm + mask_noise_mm
    # # print(121)
    return feature, feature_mm

# def add_noise(feature, feature_mm, mean=0.0, std=0.1):
#     bs = feature.size(0)
#     device = feature.device
#     noise_list = []
#     # every layer add diff noise
#     noise_lr = [x.item() for x in torch.linspace(0.4, 0, 12)]
#     for i in range(len(noise_lr)):
#         noise = torch.randn_like(feature[0, 0, :, :]) * std + mean
#         mask = torch.bernoulli(torch.full((196, 768), noise_lr[i]))
#         mask = mask.to(device)
#         mask_noise = torch.mul(noise, mask)
#         noise_list.append(mask_noise.unsqueeze(0))
#     mask_noise = torch.cat(noise_list, 0)
#     # mask_noise = mask_noise.unsqueeze(0).repeat(12, 1, 1)
#     mask_noise = mask_noise.unsqueeze(0).repeat(bs, 1, 1, 1)
#     mask_noise_mm = mask_noise[:, [0, 5, 11], :50, :]
#     feature = feature + mask_noise
#     feature_mm = feature_mm + mask_noise_mm
#     return feature, feature_mm
...
def main(args):
    ...
    if args.only_prefix:
        ...
        model = ClipCaptionModel(args.prefix_length, encoder, add_noise, clip_length=args.prefix_length_clip, prefix_size=prefix_dim,
                                 num_layers=args.num_layers, mapping_type=args.mapping_type, Timestep=args.time_step,
                                 if_drop_rate=args.if_drop_rate)
    ...
    dataset = ClipCocoDataset(args.data_root, args.data, args.features_path, 50, load_in_tmp=False, normalize_prefix=args.normalize_prefix)
    ...
    val_dataset = ClipCocoValDataset(args.data_root, args.features_path, 50, load_in_tmp=False,)
    ...
    # noise_lr = [x.item() for x in torch.linspace(0.4, 0, 30)]
