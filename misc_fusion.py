    ...
    with torch.no_grad():
        for entry_idx in range(entry_count):
            ...
            pad_image = model.module.pad_embedding.unsqueeze(0).repeat_interleave(repeats=embed.size(0),
                                                                                  dim=0).to(dtype=embed.dtype)
            pad_image_mm = model.module.pad_embedding_mm.unsqueeze(0).repeat_interleave(repeats=embed_mm.size(0),
                                                                                  dim=0).to(dtype=embed_mm.dtype) # image free vectors
            ...
            if step <= model.module.time_step:
                for i in range(step):
                    ...
                    outputs = model.module.gpt(t, inputs_embeds=generated, attention_mask=all_mask,
                                               encoder_hidden_states=embed, encoder_hidden_states_mm=embed_mm)
                    outputs1 = model.module.gpt(t, inputs_embeds=generated, attention_mask=all_mask,
                                                encoder_hidden_states=pad_image, encoder_hidden_states_mm=pad_image_mm)
